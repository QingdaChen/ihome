...
type User struct {
	...
	Password_hash string        `gorm:"size:128"json:"password_hash"` //用户密码加密的
	Mobile        string        `gorm:"size:11;unique" json:"mobile"` //手机号
	Real_name     string        `gorm:"size:32" json:"real_name"`     //真实姓名  实名认证
	Id_card       string        `gorm:"size:20" json:"id_card"`       //身份证号  实名认证
	Avatar_url    string        `gorm:"size:256" json:"avatar_url"`   //用户头像路径       通过fastdfs进行图片存储
